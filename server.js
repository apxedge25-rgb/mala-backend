...
// explain-screen: creates history and returns resumeToken
app.post("/api/v1/explain-screen", async (req, res) => {
  try {
    ...
    // Prompt: ask model to return explanation broken into segments with <SEG>
    const systemPrompt = `You are Mala, an assistant that explains on-screen content simply and clearly for a beginner. Use the user's chosen language. Keep answers short, stepwise if applicable, and avoid inventing facts. Output should include a plain explanation divided into segments separated by the token <SEG> and a one-line summary that starts with "SUMMARY:".`;
...
Instruction: Explain the above to a beginner in ${langLabel}. Break the explanation into small segments separated by "<SEG>" so the client can read them one by one. After segments, provide a one-line summary prefixed with "SUMMARY:".`;
    ...
    // Build transcript array: first element = segments list
    const transcript = [
    ...
    // initial resume token points to segmentIndex 0
...
// follow-up: user asks doubt while explanation paused
app.post("/api/v1/follow-up", async (req, res) => {
  try {
    ...
    // choose context around the pause point: last 2 segments before resumeIndex
    ...
    // system prompt instructing JSON resumeIndex output
    const systemPrompt = `You are Mala, an assistant answering a small follow-up question. Answer concisely in the user's chosen language. At the end, output a JSON object on its own line with the key "resumeIndex" which indicates the index of the next segment the client should resume from (an integer). Example: {"resumeIndex": 4}`;
...
Instruction: Answer briefly and clearly. Then output the JSON resume object on its own line.`;
    ...
    // parse last JSON line with resumeIndex
    ...
    // append user question and mala answer to transcript
